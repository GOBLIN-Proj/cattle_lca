...
class DataManager:
    ...
    def data_engine_creater(self):
        ...
        database_path = os.path.abspath(os.path.join(self.database_dir, "cattle_database.db"))
    ...
    def grass_data(self, index=None):
        ...
        if index == None:
            dataframe = pd.read_sql(
                "SELECT * FROM '%s'" % (table), self.engine
            )
    ...
    def emissions_factor_data(self, index=None):
        ...
        if index == None:
            dataframe = pd.read_sql(
                "SELECT * FROM '%s' WHERE ef_country = '%s'" % (table, self.ef_country), self.engine
            )
